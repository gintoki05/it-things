/**
 * Client-side cryptographic & generator utilities for SwissTools.
 * Zero-dependency and runs entirely in the browser.
 */

// Native SubtleCrypto for SHA families
...
export function generateUuids(
  count = 1,
  options: { uppercase?: boolean; noHyphens?: boolean } = {}
): string[] {
  const list: string[] = []
...
export function generateNanoIds(count = 1, length = 21): string[] {
  const chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_~"
  const list: string[] = []
  const safeCount = Math.min(Math.max(1, count), 100)
  const safeLength = Math.min(Math.max(6, length), 64)
  ...
  for (let i = 0; i < safeCount; i++) {
    ...
    if (typeof crypto !== "undefined" && crypto.getRandomValues) {
      ...
      for (let j = 0; j < safeLength; j++) {
        res += chars[buffer[j] % chars.length]
      }
    } else {
      for (let j = 0; j < safeLength; j++) {
